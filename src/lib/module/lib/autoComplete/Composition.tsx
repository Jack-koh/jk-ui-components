...
export function Chip(): JSX.Element {
  ...
  return (
    <ChipContent className="autocomplite__chip__field">
      ...
      {choice.include.length > 1 && (
        <div className="autocomplete__chip__additional">+{choice.include.length - 1}</div>
      )}
    </ChipContent>
  );
}
...
export function InputContainer(props: IInputContainer): JSX.Element {
  ...
  return (
    <>
      {children}
      <Input
        st={{ width: "calc(100% - 30px)", height: props.height ?? 32 }}
